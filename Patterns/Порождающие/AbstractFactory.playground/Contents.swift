...
// Фабрика создает объекты по какому-то принципу

// Фабрики используются при создании общего интерфейса
...
let littleSizeCar = littleFactory.produceCar()

let middleFactory = MiddleSizeFactory()
let middleSizeBus = middleFactory.produceBus()
